%%%% 1st month of the quarter (no gdp)

%% Initialize forecasting block
start_y = 2010; start_m = 6; %% Starting dates for the out-of-sample evaluation
end_y   = 2019;   end_m = 9; %% Ending dates for the out-of-sample evaluation
...
%nForecasts = 7;
nForecasts = 4;
    BACKGDP = zeros(forecastEvalWindow,nForecasts);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   %unb_M = logical(UnbPattern(:,:,iPanel));
    ...
    for t=start_sample:3:end_sample
        ...
        BACKGDP(t/3,:) = [dates(t) X(t,2) GDP_qnpb(end-5,1)' GDP_m(end-5,1)'] ;
    ...
    RMSENOW = sqrt( mean( ( nonzeros(BACKGDP(1:end,2)) * ones(1,2) - [nonzeros(BACKGDP(1:end,3)) nonzeros(BACKGDP(1:end,4))] ).^2 ) );
    RMSE_bench(:,:)=RMSENOW;
    disp('    RMSE of GDP BACKCASTS ')
    disp('    Qmodel   Mmodel+panel ')
    disp(RMSE_bench(:,:))
    
    RMSENOW = sqrt( mean( ( nonzeros(NOWGDP(1:end,2)) * ones(1,2) - [nonzeros(NOWGDP(1:end,3)) nonzeros(NOWGDP(1:end,4))] ).^2 ) );
    RMSE_bench(:,:)=RMSENOW;
    disp('    RMSE of GDP NOWCASTS ')
    disp('    Qmodel   Mmodel+panel ')
    disp(RMSE_bench(:,:))
    
    RMSENOW = sqrt( mean( ( nonzeros(FOR1GDP(1:end,2)) * ones(1,2) - [nonzeros(FOR1GDP(1:end,3)) nonzeros(FOR1GDP(1:end,4))] ).^2 ) );
    RMSE_bench(:,:)=RMSENOW;
    disp('    RMSE of GDP FORECASTS (1 QUARTER AHEAD) ')
    disp('    Qmodel   Mmodel+panel ')
    disp(RMSE_bench(:,:))
    
    RMSENOW = sqrt( mean( ( nonzeros(FOR2GDP(1:end,2)) * ones(1,2) - [nonzeros(FOR2GDP(1:end,3)) nonzeros(FOR2GDP(1:end,4))] ).^2 ) );
    RMSE_bench(:,:)=RMSENOW;
